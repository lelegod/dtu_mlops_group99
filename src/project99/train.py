...
def upload_to_gcs(local_path: str, gcs_path: str):
    try:
        ...
        logger.info(f"Model successfully uploaded to {gcs_path}")
...
def train(cfg: DictConfig):
    ...
    local_save_path = Path(LOCAL_MODEL_PATH)
    local_save_path.parent.mkdir(parents=True, exist_ok=True)
    
    xgb_model.save_model(str(local_save_path))
    upload_to_gcs(str(local_save_path), GCS_MODEL_PATH)
